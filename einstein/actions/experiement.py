...
class RealExperiment(Experiment):
    ...
    def get_training_data(self, unfolding=1):
        def form_mask(shape, unfolding):
            return E.tools.vstack([E.tools.ones(shape=shape, dtype=E.tools.T.config.floatX), \
                                   E.tools.zeros(shape=(unfolding - shape[0], shape[1]), dtype=E.tools.T.config.floatX)])
        ...
        if unfolding >= len(input_data):
            return [input_data], [output_data], [form_mask(output_data.shape, unfolding=unfolding)]
        else:
            ...
            return critic_train_inputs, critic_train_outputs, [None]