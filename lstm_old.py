...
def gen_data(length=LENGTH, n_batch=N_BATCH, delay=DELAY):
    ...
    y = np.arange(1, N_ITERATIONS + 1)
    # Compute y[n] = X[:, n, 0] - X[:, n - delay, 1] + noise
    # y[:, delay:, 0] += (X[:, :-delay, 1] + .01*np.random.randn(n_batch, length - delay))
...
x, y = gen_data()
x = np.split(x, N_ITERATIONS / N_BATCH)
y = np.split(y, N_ITERATIONS / N_BATCH)
...
for n in range(N_ITERATIONS):
    print "x train", x[n].reshape(10, 1,1)
    print "y train", x[n].reshape(10,1,1)
    raw_input()
    costs[n] = train(x[n].reshape(N_BATCH, LENGTH, 1), y[n].reshape(N_BATCH, LENGTH, 1))
    if not n % 10:
...
plt.ylabel('Cost')
plt.savefig('result_old.png')

print("now pridicting ...")
print y_pred(X_val)
plt.clf()
plt.plot(y_pred(X_val).reshape(10))
